  ...
  return (
    <div
      style={{
        ...
        display: "grid",
        gap: 8,
        gridTemplateColumns: "minmax(66px, 36%) minmax(0, 1fr)",
        padding: "4px 0",
      }}
    >
      <dt
...
function DiamondDetails({ diamonds }: { diamonds: EnquiryDiamond[] }) {
  const visibleDiamonds = diamonds.filter((diamond) =>
    Object.entries(diamond).some(([key, value]) => key !== "id" && hasValue(value)),
  );

  if (visibleDiamonds.length === 0) {
    return <EmptyLabel>No diamond details added.</EmptyLabel>;
  }
    ...
    <div style={{ display: "grid", gap: 10 }}>
      {visibleDiamonds.map((diamond, index) => (
        <StoneCard
          ...
          title={`Diamond ${index + 1} of ${visibleDiamonds.length}`}
        >
          <DetailRow
            label="Type"
...
function ColorStoneDetails({ stones }: { stones: EnquiryColorStone[] }) {
  const visibleStones = stones.filter((stone) =>
    Object.entries(stone).some(([key, value]) => key !== "id" && hasValue(value)),
  );

  if (visibleStones.length === 0) {
    return <EmptyLabel>No colour stone details added.</EmptyLabel>;
  }
  ...
  return (
    <div style={{ display: "grid", gap: 10 }}>
      {visibleStones.map((stone, index) => (
        <StoneCard
          key={stone.id ?? index}
          title={`Color Stone ${index + 1} of ${visibleStones.length}`}
        >
      ...
      >
        ...
        >
          ...
          <PrintSection
            ...
            count={`${item.diamonds.length} item${item.diamonds.length === 1 ? "" : "s"}`}
          >
            <DiamondDetails diamonds={item.diamonds} />
          </PrintSection>

          <PrintSection
            title="Color Stone Details"
            count={`${item.colorStones.length} item${item.colorStones.length === 1 ? "" : "s"}`}
          >
            <ColorStoneDetails stones={item.colorStones} />
          </PrintSection>
